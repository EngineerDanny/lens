#!/usr/bin/env python3
"""Repeated five-fold evaluation of absolute interaction-pair budgets in Carlstrom."""
...
REPEATS = 20
FIXED_BUDGETS = [10, 20, 50, 100, 200, 500]
...
def main() -> None:
    ...
    order = {str(value): value for value in FIXED_BUDGETS} | {"maximum": 10_000}
    summary["order"] = summary["budget_label"].map(order)
    summary = summary.sort_values("order").drop(columns="order")

    results.to_csv(
        ROOT / "results" / "carlstrom_absolute_pair_budgets_folds.csv", index=False
    )
    repeat_means.to_csv(
        ROOT / "results" / "carlstrom_absolute_pair_budgets_repeats.csv", index=False
    )
    summary.to_csv(
        ROOT / "results" / "carlstrom_absolute_pair_budgets_summary.csv", index=False
    )
    with open(
        ROOT / "results" / "carlstrom_absolute_pair_budgets_metadata.json", "w"
    ) as handle:
        json.dump({
            "seed": SEED,
            "repeats": REPEATS,
