#!/usr/bin/env python3
"""Repeated five-fold evaluation of absolute interaction-pair budgets in Schafer."""
...
REPEATS = 20
FIXED_BUDGETS = [10, 20, 50, 100, 200, 500, 1000]
...
def main() -> None:
    ...
    results.to_csv(
        ROOT / "results" / "schafer_absolute_pair_budgets_folds.csv", index=False
    )
    repeat_means.to_csv(
        ROOT / "results" / "schafer_absolute_pair_budgets_repeats.csv", index=False
    )
    summary.to_csv(
        ROOT / "results" / "schafer_absolute_pair_budgets_summary.csv", index=False
    )
    with open(
        ROOT / "results" / "schafer_absolute_pair_budgets_metadata.json", "w"
    ) as handle:
        json.dump({
            "seed": SEED,
            "repeats": REPEATS,
